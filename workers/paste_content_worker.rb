...
def process_task(task)
  #puts task.inspect
  uri = URI.parse("http://pastebin.com/api_scrape_item.php?i=" + task[:paste_id])
  puts uri.inspect
  #tor = $tor_server.get_proxy
  #ua = $tor_server.random_ua
  begin
    response = Net::HTTP.start(uri.host, uri.port) do |http|
      http.get(uri)#,  {'User-Agent' => ua})
    end
  rescue
  ...
  #  $task_server.push_task "waiting_for_content", task
  #end
  if response.body =~ /Error, we cannot find this paste/ || response.body =~ /Private Paste ID:/
  ...
  task[:data] = response.body.encode!( 'UTF-8', invalid: :replace, undef: :replace )
  #end
  #pattern = /<title>([^<]*)<\/title>/
  #response.body.scan(pattern).each do |x|
