...
while true do
  ...
  uri = URI.parse("http://pastebin.com/api_scraping.php?limit=200")
  begin
    response = Net::HTTP.start(uri.host, uri.port) do |http|
      http.get(uri.path)#(uri.path,  {'User-Agent' => ua})
    end
  rescue
  ...
  begin
    pastes = JSON.parse(response.body)
  rescue
    pastes = []
  end
  pastes.each do |paste|
    unless paste_ids.include? paste['key']
      #puts paste.inspect
      task_server.push_task("waiting_for_content", {:paste_id => paste['key'], :title => paste['title'], :ts => DateTime.now.to_s, :user => paste['user']})
      paste_ids.push paste['key']
    end
  end
